...
- Dashboard Overview no longer runs the full doctor (#16): it uses a cheap project health check that does not grow with Git worktrees. The full doctor runs Git asynchronously, four workspaces at a time, so the Dashboard stays responsive during Diagnostics. `continuity doctor` output is unchanged. Host API (breaking within version 1): `host.doctor()` now returns a Promise and must be awaited; `host.health(projectId)` is new and display-only.
