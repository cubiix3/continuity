...
- Codex 0.157 autosave (#24): interactive `codex` sessions, which Codex 0.157 hosts in its shared app-server daemon, now save by default with no setting. Launch-time `CONTINUITY_AUTOSAVE` cannot reach those hooks, so the previous opt-in never took effect there. `codex exec` (also when an agent runs it) and `codex --no-daemon` stay off, and their answers are unchanged. Code-mode edits are detected as `apply_patch`. See ADR 013.
