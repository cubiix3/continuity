...
- Context no longer starves strongly relevant memories: after current rules, a memory that covers at least half of the meaningful task terms may use up to 25% of the byte budget ahead of lower-ranked source passages. Presentation order is unchanged, and contexts without such a memory are identical.
