...
Health has two depths ([issue #16](https://github.com/cubiix3/continuity/issues/16)). The Overview reads a cheap, project-scoped `health` check derived from the same storage and registrations: capabilities, configuration, the project root and workspace Git link files. It never runs the full doctor. Diagnostics runs the full doctor, whose Git verification is asynchronous and bounded to four workspaces at a time, so the Dashboard server stays responsive. Cheap health is display state only; operations keep verifying their workspace in full. There is no separate health store or cache.
