...
- **Diagnostics:** the same storage, runtime and registration checks as `doctor`, including Git membership of every registered workspace. The Git checks run asynchronously, four workspaces at a time, and overlapping Diagnostics requests share one run, so other Dashboard requests are answered during the Git phase. The storage scan before it is synchronous and grows with the size of the database, not with worktrees. Disabled semantic retrieval is a healthy FTS-only configuration.
