...
- **Overview:** indexed-source, active-memory, quarantined-conflict and workspace-handoff counts. Active is a stored policy state; context still revalidates source-backed evidence. Its Healthy/Degraded state is a cheap check of the selected project: storage capabilities, source-scope configuration, the project root and each workspace's Git link files. It starts no Git process and runs no integrity check, so it does not slow down with many worktrees. The full check lives on Diagnostics.
...
- **Diagnostics:** the same storage, runtime and registration checks as `doctor`, including Git membership of every registered workspace. Git runs asynchronously, four workspaces at a time, so other Dashboard requests are still answered while it runs. Disabled semantic retrieval is a healthy FTS-only configuration.
