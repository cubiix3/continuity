...
/** The read-only detail tools a provider integration installs next to its hooks; saving stays with the hooks. */
...
export function createMcpServer(adapter: AgentAdapter | undefined, options: McpServerOptions = {}): McpServer {
  ...
  if (wanted.has('continuity_context')) server.registerTool('continuity_context', { ...extra, description: 'Get a task-specific context bundle for this project: current project rules, the most relevant source passages, durable memories (decisions and lessons, including ones not listed at session start) and the latest open handoff, each with provenance. Budget is UTF-8 bytes, including metadata.', inputSchema: contextRequestSchema, annotations: { ...annotations, readOnlyHint: true } }, args => result(() => adapter.context(args)));
  if (wanted.has('continuity_search')) server.registerTool('continuity_search', { ...extra, description: 'Search current sources in this project. Returns up to 10 excerpts with provenance.', inputSchema: z.object({ query: z.string().min(1).max(2000), mode: z.enum(['lexical', 'semantic', 'hybrid']).optional() }).strict(), annotations: { ...annotations, readOnlyHint: true } }, args => result(() => adapter.search(args.query, args.mode)));
