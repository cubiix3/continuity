...
program.command('doctor').description('Check SQLite integrity, migration version and FTS5').action(() => { const health = runtime().doctor(); output(health); if (health.integrity !== 'ok' || !health.fts5) process.exitCode = 1; });
...
memory.command('remember <text>').requiredOption('--key <key>', 'stable claim key').requiredOption('--source <path>', 'project-relative source path').option('--kind <kind>', 'rule, decision, memory, experience', 'memory').action((text: string, options: { key: string; source: string; kind: string }) => output(client().propose({ text, key: options.key, source_path: options.source, kind: options.kind })));
...
program.command('mcp').description('Serve five project-bound MCP tools over stdio').action(async () => {
