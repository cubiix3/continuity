...
import { handoffActive } from '../contracts.js';
...
export function buildBootstrap(input: BootstrapInput): BootstrapBundle {
  ...
  const newest = workspaceHandoffs.find(handoffActive);
  ...
  const olderHandoffs = Math.max(0, workspaceHandoffs.length - (newest ? 1 : 0));
