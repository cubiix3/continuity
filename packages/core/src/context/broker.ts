...
import type { ContextBundle, ContextItem, ContextRequest, Project, Resource, StoragePort } from '../contracts.js';
import { NamespaceGuard } from '../security/namespace.js';

export function contextBroker(storage: StoragePort, project: Project, request: ContextRequest, sources: Resource[]): ContextBundle {
  const { task, role, budget } = contextRequestSchema.parse(request);
  ...
  for (const m of storage.memories(project.project_id)) {
    if (m.status !== 'persist' || !terms.some(t => m.text.toLowerCase().includes(t))) continue;
    if (!sources.some(r => r.state === 'fresh' && r.path === m.source_path && r.hash === m.provenance.source_version)) continue;
    candidates.push({ id: m.id, kind: m.kind, content: m.text, provenance: m.provenance, reasons: ['same project', 'source-backed memory', 'source version still current', 'task term match'] });
  }
  ...
  for (const item of candidates) {
    ...
    if (size() > budget) bundle.items.pop();
  }
