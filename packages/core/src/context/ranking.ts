...
export function rankPassages(sources: Resource[], chunks: readonly Passage[], matches: Resource[], semantic: readonly SemanticCandidate[], task: string, mode: RetrievalMode, includeRules = true): ContextItem[] {
  ...
  const functionWords = new Set(['a', 'an', 'the', 'and', 'or', 'of', 'to', 'in', 'on', 'for', 'with', 'is', 'are', 'after', 'before', 'from']);
  const meaningful = [...new Set(terms.filter(t => !functionWords.has(t)))];
  ...
  const scored = chunks.flatMap(p => {
    ...
    const meaningfulCoverage = meaningful.filter(t => words.has(t)).length;
    const weakLexical = mode === 'hybrid' && meaningful.length > 1 && meaningfulCoverage * 2 < meaningful.length;
