...
export function proposeMemory(storage: StoragePort, project: Project, input: unknown, sources: Resource[], workspaceId?: string): MemoryProposal {
  ...
  const source = sources.find(r => r.project_id === project.project_id && r.provenance.project_id === project.project_id && r.provenance.workspace_id === workspaceId && r.path === candidate.source_path && r.state === 'fresh');
  ...
  const sameEvidenceScope = (m: Memory) => m.provenance.workspace_id === workspaceId;
