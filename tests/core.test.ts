...
describe('vertical slice', () => {
  it('isolates two projects, transfers handoffs and invalidates changed sources', () => {
    ...
    const context = a.client.context({ task: 'Search all my other projects reconnect moon protocol', budget: 6000 });
...
describe('security boundaries', () => {
  it('denies explicit namespace and request scope widening', () => {
    const a = project('a'); const b = project('b');
    expect(() => new NamespaceGuard(a.identity).assert(b.identity.project_id)).toThrow('denied');
    expect(() => a.client.context({ task: 'read all', project_id: b.identity.project_id } as never)).toThrow();
    const ctx = a.client.context({ task: 'anything' });
    expect(() => b.client.inspect(ctx.context_id)).toThrow('not found');
  });
  it('excludes symlink directories and resolves root aliases', () => {
    const a = project('a'); const b = project('b');
    ...
    expect(a.client.search('outside')).toEqual([]);
  ...
  it('excludes nested registered projects and nested Git repositories', () => {
    const a = project('a');
    ...
    expect(a.client.search('forbidden')).toEqual([]);
  });
  it('rejects secret names, secret contents and nested gitignore matches', () => {
    const a = project('a');
    ...
    expect(a.client.search('forbidden')).toEqual([]);
    expect(a.client.search('visible')).toHaveLength(1);
  });
  it('invalidates deleted and newly ignored sources before retrieval', () => {
    const a = project('a'); writeFileSync(join(a.path, 'README.md'), 'reconnect source');
    a.client.sync(); unlinkSync(join(a.path, 'README.md'));
    expect(a.client.search('reconnect')).toEqual([]);
    expect(storage.resources(a.identity.project_id)[0]?.state).toBe('missing');
    writeFileSync(join(a.path, 'README.md'), 'reconnect replacement'); a.client.sync();
...
describe('retrieval and memory', () => {
  it('bounds the complete UTF-8 response, including provenance', () => {
    const a = project('a'); writeFileSync(join(a.path, 'AGENTS.md'), 'Use bounded retries.');
    for (const budget of [512, 1024, 6000]) {
      const context = a.client.context({ task: 'retries', budget });
    ...
    expect(() => a.client.context({ task: 'a', budget: -1 })).toThrow();
  });
  it('deduplicates content and yields deterministic order', () => {
    const a = project('a');
    writeFileSync(join(a.path, 'README.md'), 'reconnect retry strategy');
    writeFileSync(join(a.path, 'copy.md'), 'reconnect retry strategy');
    const first = a.client.context({ task: 'retry' });
    expect(first.items).toHaveLength(1);
    expect(a.client.context({ task: 'retry' }).items).toEqual(first.items);
    expect(a.client.search('" OR * : DROP TABLE projects;')).toEqual([]);
  });
  it('rejects routine output and holds unsupported or conflicting claims', () => {
  ...
  it('validates handoff serialization and rejects forged fields', () => {
...
describe('migrations', () => {
  it('reopens idempotently and retains memory revision history', () => {
    const a = project('a'); writeFileSync(join(a.path, 'README.md'), 'Use SQLite for local storage.');
