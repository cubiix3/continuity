...
const processes = vi.hoisted(() => ({ active: 0, peak: 0, total: 0 }));
vi.mock('node:child_process', async importOriginal => {
  ...
  const execFile = (file: string, args: readonly string[], options: object, callback: (...result: unknown[]) => void) => {
    ...
    processes.active++; processes.total++; processes.peak = Math.max(processes.peak, processes.active);
...
const WORKTREES = 10;
// Real worktree creation is slow on Windows runners.
...
beforeEach(() => {
  ...
  Object.assign(processes, { active: 0, peak: 0, total: 0 });
});
